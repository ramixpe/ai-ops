...
| [docs/design/ai-space.md](docs/design/ai-space.md) | **Stretching the AI space, 2026-08-24 brainstorm** — the graduated-freedom tier ladder (descent → pinned loop → grounded recall → free-form evidence hunt → candidate narrowing), the unknown-error T3 trigger (config diff / neo4j / NetBox), and the risk register. Input to SOTA planning; no implementation authorised |
...
| [docs/build/MIGRATION.md](docs/build/MIGRATION.md) | **Standing this repo up on a new host.** Step-by-step with a check per step, written to be executed by an agent. The host-key trap is the one to read first: XRd regenerates SSH host keys on redeploy, `nettools` pins them and fails closed, and `fault_lab.py` keeps working via `AutoAddPolicy` — so the injector succeeds while the tool fails |
...
| [docs/build/DEEP-REVIEW-2026-08-27.md](docs/build/DEEP-REVIEW-2026-08-27.md) | Current deep review, peer-review reconciliation, remediation status, and residual risks |
| [docs/diagrams/](docs/diagrams/) | **Sixteen hand-authored diagrams**, one unified layer — 2026-08-23 retired the old two-layer split (a generated, byte-pinned SVG set plus a separate hand-authored presentation set); that duplication and the CI byte-pin are both gone. Built with the `diagram-design` skill, describing the system end to end **as of 2026-08-27**: two A3 faithful-detail posters (`full-system.html`, `docker-fleet.html`) covering all 102 modules and all 30 running containers individually rather than the curated ~9-node style of the rest, plus `call-lifecycle.html` — one real, already-happened investigation traced stage by stage with its actual payloads (syslog line, system prompt, a real model fabrication attempt and refusal, real device output, an actually-rendered Telegram card). **These are hand-authored, not generated — they can drift from the code and are only as current as their last update.** Start at [`docs/diagrams/index.html`](docs/diagrams/index.html) |
