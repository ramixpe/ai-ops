...
#: Verbatim from `agent_nettools.template_parsers._BGP_SOCKET`, and it must stay
#: verbatim. Anchored, with three *positional* named groups, so there is no
#: spelling of this line that lets a search land on the wrong field. Round 8's
#: `socket.*?(armed|not armed)` is the defect this exists to make impossible.
...
def verdict() -> dict:
    ...
    if not v["instrument"]["baseline_trustworthy"]:
        v["note"] = (
