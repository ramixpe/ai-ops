#!/usr/bin/env python3
"""Measure structured, deferred, presentation, and unaccounted parser coverage."""
...
class Auditor:
    ...
    def finalize(self, original: Callable[..., dict[str, Any]], **kwargs: Any) -> dict[str, Any]:
        raw = kwargs["raw"]
        consumed = Counter(line.strip() for line in kwargs.get("consumed", ()))
        ...
        for segment in raw.splitlines(keepends=True):
            ...
            if any(rule.matches(line) or rule.matches(stripped) for rule in template_parsers.XR_COMMON_IGNORES):
...
def traced_finalizers(auditor: Auditor) -> Iterable[None]:
    ...
    for module, original in originals.items():
        module.finalize = lambda _original=original, **kwargs: auditor.finalize(_original, **kwargs)  # type: ignore[attr-defined]
    try:
        yield
    finally:
        for module, original in originals.items():
        ...
        for label_dir in sorted({path.parent for path in corpus.rglob("*.txt")}):
            ...
            for path in sorted(label_dir.glob("*.txt")):
                ...
                if intent is not None:
                    ...
                    _parsed, status = parsers.parse_intent("cisco_xr", intent, {path.name: path.read_text()})
                elif template is not None:
                    auditor.current_name = f"template:{template}"
                    _parsed, status = template_parsers.parse_template_output("cisco_xr", template, path.read_text())
                else:
                ...
                seen.add(path)
    missing = sorted(set(corpus.rglob("*.txt")) - seen)
...
def report(auditor: Auditor) -> dict[str, Any]:
    ...
    def enrich(counts: Counts) -> dict[str, Any]:
        data = asdict(counts)
        for bucket in ("structured", "deferred", "presentation", "unaccounted"):
            data[f"{bucket}_line_pct"] = _pct(data[f"{bucket}_lines"], counts.meaningful_lines)
            data[f"{bucket}_byte_pct"] = _pct(data[f"{bucket}_bytes"], counts.raw_bytes)
        return data
...
def main() -> int:
    ...
    parser.add_argument("--corpus", type=Path, default=ROOT / "tests" / "fixtures" / "cisco_xr")
    ...
    if args.json:
        ...
        return 0
    total = result["total"]
    print(f"{total['files']} files / {total['cases']} parser cases / {total['raw_bytes']} bytes / {total['meaningful_lines']} non-blank lines")
    print("parser                         structured   deferred  presentation  unaccounted")
    for name, row in result["by_parser"].items():
        print(f"{name:30} {row['structured_line_pct']:9.2f}% {row['deferred_line_pct']:9.2f}% {row['presentation_line_pct']:12.2f}% {row['unaccounted_line_pct']:11.2f}%")
    print(f"TOTAL (lines)                 {total['structured_line_pct']:9.2f}% {total['deferred_line_pct']:9.2f}% {total['presentation_line_pct']:12.2f}% {total['unaccounted_line_pct']:11.2f}%")
    print(f"TOTAL (bytes)                 {total['structured_byte_pct']:9.2f}% {total['deferred_byte_pct']:9.2f}% {total['presentation_byte_pct']:12.2f}% {total['unaccounted_byte_pct']:11.2f}%")
    return 0
