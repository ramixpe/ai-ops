...
Two caveats: the tool manifest needs the consolidation described in the LLD (§4.1 — the staged surface now EXISTS behind `NETTOOLS_MCP_SURFACE` (B-479); classic carries 23 tools today), and MCP clients are a developer-shaped surface rather than a NOC-shaped one. But if the near-term goal is "let someone poke at this interactively this week", it is by far the cheapest path and it is already written.
