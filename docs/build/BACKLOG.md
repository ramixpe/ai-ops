...
| **B-206a** | **Device logging severity levels are dropping the events the historical axis exists for** | Only `err`(3) and `warning`(4) reach the collector. `%BGP-5-ADJCHANGE` and IS-IS adjacency transitions are severity 5 and **never arrive at all** — so the state changes correlation exists to explain are absent from the corpus, whatever query is written over it. syslog-ng applies no severity filter; the floor is on the routers. **A platform fix, not a Loki limitation** | T-004 finding (OBS-014) · Q-011 | S | D8 |
