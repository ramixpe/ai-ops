...
| **B-107** | **Flow: `isis_adjacency`** | The most-shared dependency — every protocol above it descends through it **Its broken state is designed alongside the flow, not captured afterwards** — a healthy-only corpus has structurally zero coverage of the broken case, not weak coverage. The `broken` label caught three separate classes of defect the healthy fixtures could not reach: four parser gaps, the walk-semantics bug (Q-017), and the subject-vocabulary gap. See OBS-050, OBS-055, OBS-057. | MVP-0 complete | S | D5 |
| **B-108** | **Flow: `device_health`** | Wraps the existing `health.py` verdicts as a flow, so "is PE1 ok" has an entry point **Its broken state is designed alongside the flow, not captured afterwards** — a healthy-only corpus has structurally zero coverage of the broken case, not weak coverage. The `broken` label caught three separate classes of defect the healthy fixtures could not reach: four parser gaps, the walk-semantics bug (Q-017), and the subject-vocabulary gap. See OBS-050, OBS-055, OBS-057. | B-107 | S | D5 |
| **B-109** | **Flow: `ldp_session`** | Completes the MPLS control plane **Its broken state is designed alongside the flow, not captured afterwards** — a healthy-only corpus has structurally zero coverage of the broken case, not weak coverage. The `broken` label caught three separate classes of defect the healthy fixtures could not reach: four parser gaps, the walk-semantics bug (Q-017), and the subject-vocabulary gap. See OBS-050, OBS-055, OBS-057. | B-107 | M | D5 |
| **B-110** | **Flow: `l3vpn_service`** | The one object type whose identity is not obvious — needs the naming scheme first **Its broken state is designed alongside the flow, not captured afterwards** — a healthy-only corpus has structurally zero coverage of the broken case, not weak coverage. The `broken` label caught three separate classes of defect the healthy fixtures could not reach: four parser gaps, the walk-semantics bug (Q-017), and the subject-vocabulary gap. See OBS-050, OBS-055, OBS-057. | T-006 finding | M | D5 |
| **B-111** | **Flow: `topology`** | Multi-device; the fabric's LLDP data is self-contradictory, so this one must report disagreement rather than assert links **Its broken state is designed alongside the flow, not captured afterwards** — a healthy-only corpus has structurally zero coverage of the broken case, not weak coverage. The `broken` label caught three separate classes of defect the healthy fixtures could not reach: four parser gaps, the walk-semantics bug (Q-017), and the subject-vocabulary gap. See OBS-050, OBS-055, OBS-057. | T-006 | M | D5 |
