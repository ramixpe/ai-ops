...
> **Archived 2026-08-20, ahead of v1.0.0.** This file now holds only the
> historical task list (Parts 1–8) — 34 tasks, all long since executed and
> reviewed at M4 (`docs/build/MVP0-REVIEW.md`, before this file itself moved to
> `docs/archive/`). **PART 0 — RULES OF ENGAGEMENT, which was here, is not
> historical** and has moved to [`docs/build/PROCESS.md`](../build/PROCESS.md)
> — still live, still binding, same section numbers (§0.1–§0.16) so that every
> existing `` `BUILD-PLAN.md` §0.N `` citation across the tree resolves to the
> same rule, just at a new address.
